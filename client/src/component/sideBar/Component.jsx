...
import { IoSearchCircleOutline, IoSearchCircleSharp, IoSettingsSharp, IoSettingsOutline } from 'react-icons/io5';
...
const SideBarComp = ({ theme, changeTheme, navigate, registeringNFT, setRegisteringNFT }) => {
    ...
    return (
        <SideBarArea>
            ...
            >
                {location.pathname == '/' ? <AiFillHome size={'25'} /> : <AiOutlineHome size={'25'} />} <p>홈</p>
            ...
            >
                <ThemeBtn size={'25'} innerText={`${theme == 'dark' ? '밝은 모드' : '어두운 모드'}`}></ThemeBtn>
            </SideItem>
            <>
                <SideItem
                    theme={theme}
...
const SideItem = styled.div`
    ...
    cursor: pointer;
    &:hover {
        background-color: ${(props) => (props.theme == 'dark' ? '#5a5a5a' : '#e0e0e0')};
    }
    > p {
...
const PopItem = styled.div`
    width: 100%;
    border-radius: 5px;
    background-color: ${(props) => (props.theme == 'dark' ? '#1e1e2e' : '#d4d3d3')};
    a {
